...
def monitor_wakeup_pin():

    # Setup for GPIO (using BCM numbering)
    GPIO.setmode(GPIO.BCM)

    GPIO.setup(WAKEUP_GPIO_PIN, GPIO.IN)

    default_value = GPIO.input(WAKEUP_GPIO_PIN)
    last_activated_value = default_value

    """Monitor the R503 wakeup pin."""
    while True:
        _input = GPIO.input(WAKEUP_GPIO_PIN)
        
        
        if _input != default_value:
            last_activated_value = _input
            # print(_input)
            print("Wakeup pin activated")

            while True:
                _input = GPIO.input(WAKEUP_GPIO_PIN)
                if last_activated_value != _input:
                    break
                time.sleep(0.01)
...
if __name__ == '__main__':
    ...
    thread = threading.Thread(target=monitor_wakeup_pin)
    thread.daemon = True
    thread.start()
